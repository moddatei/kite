...
#[command(
    name = "kitectl",
    version = "0.1.0",
    about = "Kite Mesh Protocol CLI & Diagnostics"
)]
struct Cli {
    #[command(subcommand)]
    command: Commands,
...
fn main() {
    ...
    match cli.command {
        Commands::Status => {
            ...
            println!(
                "{}",
                "   KITE MESH NODE STATUS (kited-local)        "
                    .bold()
                    .cyan()
            );
            println!(
            ...
            println!(
                ...
                "1 (Virtual RF / mon0)".white()
            );
            println!(
                "{:<24} : {}",
                "Discovered Neighbors",
                "4 peers in RF range".bright_blue()
            );
            println!(
                "{:<24} : {}",
                "Buffered Bundles",
            ...
            println!(
                "{:<18} {:<10} {:<10} {:<8}",
                "PEER ADDRESS", "RSSI", "DELIVERY", "HOPS"
            );
            println!(
                "{:<18} {:<10} {:<10} {:<8}",
                "0x7766554433221100", "-62 dBm", "98.4%", "1"
            );
            println!(
                "{:<18} {:<10} {:<10} {:<8}",
                "0xbbaabbccddeeff01", "-78 dBm", "89.1%", "1"
            );
            println!(
                "{:<18} {:<10} {:<10} {:<8}",
                "0xfeedfacedeadbeef", "-85 dBm", "74.0%", "2"
            );
        }
        ...
        Commands::Whisper { dst, message, ttl } => {
            ...
            println!(
                "{}",
                "✓ Whisper bundle constructed successfully".bold().green()
            );
        ...
        Commands::DumpBuffer => {
            println!(
                "{}",
                "=== Opportunistic Store-Carry-Forward Ring Buffer ===".bold()
            );
        ...
        Commands::Keygen => {
            ...
            println!(
                "{}",
                "Generated new Kite Cryptographic Identity:".bold().green()
            );
            println!("  Public Key (NodeId) : {}", node_id.to_string().cyan());
            println!(
                "  Short Address       : {}",
                node_id.to_short_address().to_string().yellow()
            );
